...
//   const [isParsedViewModalOpen, setIsParsedViewModalOpen] = useState(false);
  ...
  useEffect(() => {
    ...
    getBackendSrv()
      .get(`api/plugins/qxip-flow-panel/settings`)
      .then((data: any) => {
  ...
  return (
    <Modal title="Message Details" isOpen={modalIsOpen} onDismiss={onModalClose}>
      <Tooltip content={`Displays a parsed view of the message`} placement="top">
        <span
          style={{
            display: 'flex',
      ...
      {!isParsed &&
        modalData &&
        Object.entries(modalData).map((item: any, index: number) => (
          // <p>{item} | {key}</p>
